...
class CreateProductPage {
	...
	async createProduct(data: { name: string; description: string; price: string; quantity: string }) {
...
class UpdateProductPage {
	...
	async updateProduct(data: { name: string; description: string; price: string }) {
		await this.nameInput().fill(data.name);
		await this.descriptionInput().fill(data.description);
		await this.priceInput().fill(data.price);
...
async function mockCreateEditProductApi(page: Page) {
	...
	const extractFromMultipart = (rawBody: string, field: string) => {
