...
describe("Login", () => {
	...
	it("renders login form with required inputs", () => {
		...
		expect(screen.getByPlaceholderText(/(enter\s+your\s+)?email/i)).toBeInTheDocument();
		expect(screen.getByPlaceholderText(/(enter\s+your\s+)?password/i)).toBeInTheDocument();
		expect(screen.getByRole("button", { name: /log\s?in/i })).toBeInTheDocument();
		expect(screen.getByRole("button", { name: /Forgot\s?Password\?/i })).toBeInTheDocument();
	});

	it("renders all fields empty on initial load", () => {
		render(<Login />);

		expect(screen.getByPlaceholderText(/(enter\s+your\s+)?email/i)).toHaveValue("");
		expect(screen.getByPlaceholderText(/(enter\s+your\s+)?password/i)).toHaveValue("");
	});
	...
	it("navigates to forgot-password when user clicks forgot password button", () => {
		...
		fireEvent.click(screen.getByRole("button", { name: /Forgot\s?Password\?/i }));
