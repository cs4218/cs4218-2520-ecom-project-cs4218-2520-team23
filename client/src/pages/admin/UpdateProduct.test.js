...
describe("UpdateProduct", () => {
	...
	describe("Rendering", () => {
		...
		it("pre-populates all fields with the fetched product data", async () => {
			...
			expect(screen.getByPlaceholderText("write a description")).toHaveValue("A great product");
		...
		it("shows photo preview when a new file is selected", async () => {
			...
			expect(screen.getByAltText("product_photo")).toHaveAttribute("src", "blob:mock-url");
		});
		...
		it("shows filename on upload label after file is selected", async () => {
			...
			expect(screen.getByText((content) => content.includes("new-photo.png"))).toBeInTheDocument();
		});
