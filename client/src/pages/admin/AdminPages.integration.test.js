...
jest.mock("antd", () => {
	const Select = ({ children, onChange, placeholder, value }) => (
		<select aria-label={placeholder} onChange={(e) => onChange(e.target.value)} value={value}>
...
function fillCreateProductForm({ name = "New Product" } = {}) {
...
describe("Admin Pages Integration Tests", () => {
	...
	describe("Create Product - API Integration", () => {
		...
		test("Form submission calls POST endpoint with FormData", async () => {
			...
			// Fill in form fields
			fillCreateProductForm({ name: "New Product" });
		...
		test("Successful create shows toast and navigates", async () => {
			...
			fillCreateProductForm({ name: "Created Product" });
			...
			expect(screen.getByTestId("location")).toHaveTextContent("/dashboard/admin/products");
		});
		...
		test("Failed create shows error toast and stays", async () => {
			...
			fillCreateProductForm({ name: "Bad Product" });
			...
			expect(screen.getByTestId("location")).toHaveTextContent("/dashboard/admin/create-product");
		});
		...
		test("BUG TEST: If axios.post is not called, form submission doesn't work", async () => {
			...
			fillCreateProductForm({ name: "Test" });
	...
	describe("Update Product - useParams and API Integration", () => {
		...
		test("Form fields are pre-populated with fetched product data", async () => {
			...
			expect(await screen.findByDisplayValue("Original Product")).toBeInTheDocument();
		...
		test("Form submission sends PUT request with product ID", async () => {
			...
			await waitFor(() => {
				expect(axios.put).toHaveBeenCalledWith("/api/v1/product/update-product/prod-123", expect.any(FormData));
			});
		});
		...
		test("Failed update shows error toast and stays", async () => {
			...
			expect(screen.getByTestId("location")).toHaveTextContent("/dashboard/admin/update-product/test-slug");
		});
	});
	...
	describe("Critical Logic Tests", () => {
		...
		test("if axios.put is not called, product won't be updated", async () => {
			...
			await waitFor(() => {
				expect(axios.put).toHaveBeenCalledWith("/api/v1/product/update-product/p1", expect.any(FormData));
			});
		});
