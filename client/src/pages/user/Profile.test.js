...
describe("client/src/pages/user/Profile.js (unit)", () => {
	...
	test("prefills form fields from auth.user and email input is disabled", async () => {
		...
		const nameInput = screen.getByPlaceholderText(/(enter\s+your\s+)?name/i);
		const emailInput = screen.getByPlaceholderText(/(enter\s+your\s+)?email/i);
		const phoneInput = screen.getByPlaceholderText(/(enter\s+your\s+)?phone/i);
		const addressInput = screen.getByPlaceholderText(/(enter\s+your\s+)?address/i);
	...
	test("typing updates controlled inputs (name/phone/address)", async () => {
		...
		const phoneInput = screen.getByPlaceholderText(/(enter\s+your\s+)?phone/i);
		const addressInput = screen.getByPlaceholderText(/(enter\s+your\s+)?address/i);
		const passwordInput = screen.getByPlaceholderText(/(enter\s+your\s+)?password/i);
	...
	test("submit success: calls axios.put with payload, updates auth + localStorage, shows success toast", async () => {
		...
		await waitFor(() => expect(screen.getByPlaceholderText(/(enter\s+your\s+)?name/i)).toHaveValue("Alice"));
