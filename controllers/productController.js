...
const ALLOWED_PHOTO_MIME_TYPES = new Set(["image/jpeg", "image/jpg", "image/png", "image/webp", "image/gif"]);

const isAllowedPhotoType = (mimeType) => {
	return typeof mimeType === "string" && ALLOWED_PHOTO_MIME_TYPES.has(mimeType.toLowerCase());
};
...
export const createProductController = async (req, res) => {
	try {
		...
		switch (true) {
			...
			case !isAllowedPhotoType(photo.type):
				return res.status(500).send({ error: "Invalid photo type" });
		}
...
export const updateProductController = async (req, res) => {
	try {
		...
		switch (true) {
			...
			case photo && !isAllowedPhotoType(photo.type):
				return res.status(500).send({ error: "Invalid photo type" });
		}
